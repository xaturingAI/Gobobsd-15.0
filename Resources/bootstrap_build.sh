...
if [ ! \( -x ${bootstrapDest}/bin/gsed -a -L ${bootstrapDest}/bin/sed \) ]; then
  ...
  if [ ! -x ${bootstrapDest}/bin/gsed ]; then
    if [ ! -d ${sourcesDir}/sed-4.9 ]; then
      tar -xzf ${archivesDir}/sed-4.9.tar.gz -C ${sourcesDir}
    fi
    cd ${sourcesDir}/sed-4.9
...
if [ ! -x ${bootstrapDest}/bin/python ]; then
  ...
  echo Python
  echo ---------------------------------------------------
  if [ ! -d ${sourcesDir}/Python-3.11.0 ]; then
    tar -xzf ${archivesDir}/Python-3.11.0.tgz -C ${sourcesDir}
  fi
  cd ${sourcesDir}/Python-3.11.0
  ./configure --prefix=$bootstrapDest --enable-optimizations && make && make install
