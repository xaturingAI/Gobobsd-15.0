...
# # export PYTHONPATH="${ROOT}/System/Links/Libraries/python3.11/site-packages:${ROOT}/System/Links/Libraries/python3.0/site-packages:${ROOT}/System/Links/Libra>
...
# Post-install work
echo "import site" > ${bootstrapDest}/lib/python3.11/sitecustomize.py
echo 'site.addsitedir("/System/Links/Libraries/python3.11/site-packages/")' >> ${bootstrapDest}/lib/python3.0/sitecustomize.py
echo 'site.addsitedir("/System/Links/Libraries/python3.11/site-packages/")' >> ${bootstrapDest}/lib/python3.0/sitecustomize.py
echo 'site.addsitedir("/System/Links/Libraries/python3.11/site-packages/")' >> ${bootstrapDest}/lib/python3.0/sitecustomize.py
echo 'site.addsitedir("/System/Links/Libraries/python3.11/site-packages/")' >> ${bootstrapDest}/lib/python3.0/sitecustomize.py

# Change shell to bash for live user
pw usermod live -s bash
...
# Apply Compile patches if any exist
cd ${ROOT}/Programs
COMPILE_DIR=$(ls -d Compile/*/ 2>/dev/null | head -n 1)
if [ -n "$COMPILE_DIR" ] && [ -d "$COMPILE_DIR" ]; then
    cd "$COMPILE_DIR"
    COMPILEPROGDIR=$(pwd)
    COMPILEPROGVER=$(basename $COMPILEPROGDIR)

    set -f
    COMPILEPATCHES="Compile-${COMPILEPROGVER}-*.patch"
    set +f
    COMPILEPATCHESEXP=$(cd ${bootstrapScriptsDir}/; echo ${COMPILEPATCHES})
    if [ "x${COMPILEPATCHESEXP}" != "x${COMPILEPATCHES}" ]; then
        for f in ${COMPILEPATCHESEXP}; do
            echo "Applying patch: $f"
            patch -Np1 < ${bootstrapScriptsDir}/$f
        done
    fi
    unset COMPILEPATCHESEXP
fi
